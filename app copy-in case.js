...
import { StreamBuffer } from './decoder.js';   // still needed for modelToKTMODEL
    ...
    try {
        ...
        lastContextMenu = showContextMenu(clientX, clientY, menu => {
            ...
            if (isCubeModel) {
                ...
                menu.addButton('Export as .ktm', null, () => {
                    const id        = Number(target.parentNode.id.slice(2));
                    const cubeModel = jsonData.WorldObjects.find(o => o.Id === id);
                    const proto     = jsonData.Prototypes.find(p => p.Id === cubeModel.Data.protoTypeID[0]);

                    const popup = showPopup(`
                        <h3 style="font-family:'NEON',sans-serif;">Download File</h3>
                        <input id="dl-name"  class="popup-input" placeholder="File name…">
                        <input id="dl-scale" class="popup-input" type="number" value="1">
                        <button class="btn-type1" id="dl-btn">Download</button>
                    `);
                    document.getElementById('dl-btn').addEventListener('click', () => {
                        const name  = document.getElementById('dl-name').value  || 'SnapXFile';
                        const scale = Number(document.getElementById('dl-scale').value);
                        popup.delete();
                        downloadBytes(modelToKTMODEL(scale, proto.Data), name + '.ktm');
                    });
                });
            }
...
function modelToFV(cubes) {
    ...
    cubes.forEach((cube, i) => {
        ...
        const n = k => base + k;
        const c = CUBE_COLORS[materials[0]];
        // 6 quad faces → will be triangulated later
        [[4,5,6,7],[0,1,2,3],[0,1,6,7],[1,2,5,6],[0,3,4,7],[2,3,4,5]]
            .forEach(q => faces.push([...q.map(n), c]));
    });
...
function modelToKTMODEL(scale, cubes) {
    const buf = new StreamBuffer();
    buf.WriteByte(7);
    buf.WriteString('KTMODEL', false);
    buf.WriteFloat(scale ?? 0.05, false);
    for (const cube of cubes) {
        buf.WriteShort(cube.x, false);
        buf.WriteShort(cube.y, false);
        buf.WriteShort(cube.z, false);
        buf.Write(cube.materials ?? Array(6).fill(cube.material ?? 21));
        buf.Write(cube.corners   ?? [20, 120, 124, 24, 4, 104, 100, 0]);
    }
...
function addModelCanvas(parent, data) {
    ...
    faces.forEach(face => {
        face.slice(0, -1).forEach(idx => {
            positions.push(...vertices[idx]);
            colors.push(...hexToRGBA(face[face.length - 1]));
        });
    });
    ...
    scene.add(new THREE.Mesh(geo, new THREE.MeshLambertMaterial({ vertexColors: true, side: THREE.DoubleSide, transparent: true })));
